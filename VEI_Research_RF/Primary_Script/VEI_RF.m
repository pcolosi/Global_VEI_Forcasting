...
%Import the eruption data from the data file
eruption_data =readtable('GVP_Eruption_Results_Clean_3.csv');

%Import the volcano data catelog from the data file
        ...
        for i_bol = 0:1
            ...
            for k=1:length(Volcano),
                ...
                if (length(Volcano(k).eruption)>1)
                    ...
                    %Calculate median, mode, min, max 
                    %Predictors(1)= round(mean([Volcano(k).eruption(2:end).vei]));
                    Predictors(1)= [Volcano(k).eruption(2).vei];
                    ...
                    Volcano(k).medianVEINR=(median([Volcano(k).eruption(2:end).vei]));
            ...
            for iy = 1:5
                ...
                if iy == 1
                    VEI = [Volcano.secondVEI]';
                    
                elseif iy == 2
                    VEI = [Volcano.medianVEINR]';
                ...
                while iy == 1
                    syn_predictorVEI = [syn_predictorVEI_matrix.secondVEI]';
                    break
                end
...
%% Create tables with Volcano names within the Tables of Attributesfor Accuracy and RMSE
...
for iVEI=0:3
    ...
    for itime = 0:1
        ...
        for i_bol = 0:1
            ...
            %Use Create_table_attributes_ED Function
            [Tbl_VEI_allcat,Tbl_VEI_cat,Tbl_VEI_dom,Tbl_VEI_etd,Tbl_VEI_ed,Tbl_VEI_morph,Tbl_VEI_num,Tbl_VEI_tec,Tbl_unhistVEI_cat,Tbl_unhistVEI_dom,Tbl_unhistVEI_morph,Tbl_unhistVEI_tec] = Create_table_attributes_Name(eruption_data_clean_nan,volcano_data);
            ...
            %Save Workspace 
            matlab.io.saveVariablesToScript(strcat(master_direct,'/VEI_Research/Table_Attributes_Script_Name/Tablescript_Varibles_Names_',num2str(iVEI),'_',time_thresh(itime+1),'_',bol_diff(i_bol+1),'.m'))
...
if retrainflag == 1
    ...
    Script_names = ["0_after_1500_threshold","0_all_time_threshold","1_after_1500_threshold","1_all_time_threshold","2_after_1500_threshold","2_after_1500_remove","2_all_time_threshold","2_all_time_remove","3_after_1500_threshold","3_all_time_threshold",];
...
if retrainflag == 1
    ...
    %Import the Training Model to Access the Accuracy 
    %Load a training model for each attribute and then save the type of traing
    %model and the accuracy which will be save in a varible to continues to
    %increase each time 
...
%names
Accuracy_name_concat = categorical({'All Attributes','Intrinsic Properties of Volcano','Dominant Petrology','Repose Time','Eruption Duration','Morphology','All Numerical Attributes','Tectonic Setting','Second VEI','Median VEI','Mode VEI','Minimum VEI','Maximum VEI'});

%Reorder the Names for the plot
Accuracy_name_concat_F = reordercats(Accuracy_name_concat,{'All Attributes','Intrinsic Properties of Volcano','Dominant Petrology','Repose Time','Eruption Duration','Morphology','All Numerical Attributes','Tectonic Setting','Second VEI','Median VEI','Mode VEI','Minimum VEI','Maximum VEI'});
...
xl2 = xline(8.5,'--k','Random Forest Models','FontSize',18);
hold off

[height_Accuracy_num_final,~,~]=size(Accuracy_num_final) ;
for l = 1:height_Accuracy_num_final 
...
%Plot vertical line
xl1 = xline(8.5,'--k','Simple Prediction Models','FontSize',18);
xl2 = xline(8.5,'--k','Random Models','FontSize',18);
hold off
%Changing Aspects of Dashed Line 
xl1.LabelOrientation = 'horizontal';
xl2.LabelHorizontalAlignment = 'left';
...
RMSE_name_concat = categorical({'All Attributes','Intrinsic Properties of Volcano','Dominant Petrology','Repose Time','Eruption Duration','Morphology','All Numerical Attributes','Tectonic Setting','Second VEI','Median VEI','Mode VEI','Min VEI','Max VEI'});

%Reorder the Names for the plot
RMSE_name_concat_F = reordercats(RMSE_name_concat,{'All Attributes','Intrinsic Properties of Volcano','Dominant Petrology','Repose Time','Eruption Duration','Morphology','All Numerical Attributes','Tectonic Setting','Second VEI','Median VEI','Mode VEI','Min VEI','Max VEI'});
